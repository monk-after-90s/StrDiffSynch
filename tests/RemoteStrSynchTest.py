...
class RemoteStrSynchTest(TestCase):
    ...
    def test_1_handle_remote_synch_command_and_handle_local_synch_command_intial_synch(self):
    ...
    def test_3_remote_hash_str_history(self):
        ...
        remote_synch_data = self.synch_box1.handle_remote_synch_command(self.box2_history[-1].hash)
        self.assertEqual(tuple, type(remote_synch_data))
        StrDiff.create_str_diff_from_metadata(remote_synch_data)
        self.synch_box2.handle_local_synch_command(remote_synch_data)
        self.assertEqual(self.synch_box1.local_str, self.synch_box2.local_str)
    ...
    def test_5_strdiff_add_error_handler(self):
        async def test_5_strdiff_add_error_handler():
            ...
            self.synch_box2.handle_local_synch_command(remote_synch_data, strdiff_add_error_handler)
            self.assertEqual(self.synch_box1.local_str, self.synch_box2.local_str)
            ...
            await self.synch_box2.handle_local_synch_command(remote_synch_data, strdiff_add_error_handler)
            self.assertEqual(self.synch_box1.local_str, self.synch_box2.local_str)
    ...
    def test_6_mutual_synchronization(self):
        ...
        # box2同步远程，即box1
        remote_synch_data = self.synch_box2.handle_remote_synch_command(self.synch_box1.local_str_hash)
        self.assertEqual(tuple, type(remote_synch_data))
        StrDiff.create_str_diff_from_metadata(remote_synch_data)
        self.synch_box1.handle_local_synch_command(remote_synch_data)
        self.assertEqual(self.synch_box1.local_str, self.synch_box2.local_str)

    def test_7_refetch_full_raw_string(self):
        # 用假的哈希值获取差异值
        remote_synch_data = self.synch_box2.handle_remote_synch_command('hj938d2y38iw81')
